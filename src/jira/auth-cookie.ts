...
function decryptChromeLinuxValue(encrypted: Buffer, safeStoragePassword: string): string {
...
export async function getJiraCookieHeader(
  ...
  const safeStoragePassword = await keytar.findPassword(keyringService);
  ...
  try {
    ...
    try {
      const rows = db
        .prepare(
          `SELECT name, encrypted_value, expires_utc FROM cookies WHERE host_key LIKE ?`,
        )
        .all(`%${domain}%`) as { name: string; encrypted_value: Buffer; expires_utc: number }[];
